...
class QPTMessageQueue(qtc.QObject):
    ...
    @qtc.pyqtSlot()
    def q_move_to(self, pan, tilt, move_type):
        if self.qpt_connected and not self.q.full():
            self.q.put_nowait(QPTMessage(1, ['MoveTo', pan, tilt, move_type]))
